...
coancestryCoeff <- function(xmlText, Labels=NULL, timeAttr) {
  ...
  #============== Revised section
  coancestry_matrix <- numericMatrix[nrow(numericMatrix):1, ncol(numericMatrix):1]
  
  if (!is.null(Labels)) {
    Labels <- as.vector(Labels)
    rownames(coancestry_matrix) <- rev(Labels[seq_len(nrow(coancestry_matrix))])
    colnames(coancestry_matrix) <- Labels[seq_len(ncol(coancestry_matrix))]
  }
  
  return(list(
    type = "coancestry_coefficients",
    title = "Matrix of Coancestry Coefficients",
    time = timeAttr,
    data = coancestry_matrix,
    long = as.data.frame(as.table(coancestry_matrix))
  ))
