...
pairFstMatrix <- function(xmlText, Labels=NULL, timeAttr){
  ...
  #============== Revised section
  fstMatrix <- numericMatrix[nrow(numericMatrix):1, ncol(numericMatrix):1]
  
  if (!is.null(Labels)) {
    Labels <- as.vector(Labels)
    rownames(fstMatrix) <- rev(Labels[seq_len(nrow(fstMatrix))])
    colnames(fstMatrix) <- Labels[seq_len(ncol(fstMatrix))]
  }
  
  return(list(
    type = "pairwise_fst",
    title = "Matrix of pairwise FST",
    time = timeAttr,
    data = fstMatrix,
    long = as.data.frame(as.table(fstMatrix))
  ))
